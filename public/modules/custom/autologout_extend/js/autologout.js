...
(function ($, Drupal, cookies) {
  ...
  Drupal.behaviors.autologout = {
    attach: function (context, settings) {
      ...
      // Fire up dialog right away if needed.
      if (localSettings.forceDialog) {
        dialog();
      }
      ...
      if (localSettings.refresh_only) {
        ...
        if (localSettings.logout_regardless_of_activity) {
          ...
          // and log them out.
          if (localSettings.overridenTimeout) {
            t = setTimeout(function () { init(true)}, localSettings.overridenTimeout);
          } else {
      ...
      function getAjaxPath(path) {
        if (drupalSettings.autologout.usePathPrefix) {
          return drupalSettings.path.baseUrl + drupalSettings.path.pathPrefix + path
        } else {
          return drupalSettings.path.baseUrl + path
        }
      }
      ...
      Drupal.Ajax['autologout.getTimeLeft'] = Drupal.ajax({
        ...
        url: getAjaxPath('autologout_ajax_get_time_left'),
        submit: {
          uactive : activity
        },
       ...
       */
      Drupal.Ajax.prototype.autologoutRefresh = function (timerfunction, refreshSession = false) {
        var ajax = this;
        ajax.options.data.refresh = refreshSession;
        ajax.options.success = function (response, status) {
          if (typeof response === 'string') {
            response = $.parseJSON(response);
          }
      ...
      Drupal.Ajax['autologout.refresh'] = Drupal.ajax({
        base: null,
        element: document.body,
        url: getAjaxPath('autologout_ajax_set_last'),
      ...
      function refresh(sessionRefresh = false) {
        Drupal.Ajax['autologout.refresh'].autologoutRefresh(init, sessionRefresh);
      }
